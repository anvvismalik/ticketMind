...
def load_dataset(df: pd.DataFrame, mapping: dict, source_name: str = "uploaded") -> dict:
    ...
    log = AuditLog(
        ...
        reasoning=f"Loaded {processed} tickets from '{source_name}'. KB: {kb_size_before} → {kb_size_after}",
        confidence=1.0
    )
    session.add(log)
    session.commit()
